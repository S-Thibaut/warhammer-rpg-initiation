...
export const Rules = () => {
	return (
		<div className="Rule1">
			...
			<p>
				<strong>AVERTISSEMENT :</strong> Tout ce qui est expliqué sur cette page
				est automatisé par la plateforme de jeu de rôle. Un simple clic suffira
				pour lancer le dé correspondant et calculer le résultat. Ces
			...
			<p>
				...
				lignes des règles afin que vous puissiez comprendre les mécaniques et
				les possibilités qui s’offrent à vous. Vous allez vous appuyer sur une
				...
				Pour déterminer une réussite ou un échec, vous effectuerez un jet sur
				cette échelle de valeur, c’est-à-dire l’équivalent d’un
				<strong> dé à 100 faces</strong> — oui, ça existe ! Dans la pratique,
				nous utiliserons <strong>deux dés à 10 faces</strong> : l’un pour
				représenter les dizaines et l’autre pour les unités.
			</p>
			...
			<p>
				Cela vous permet de savoir dans quels domaines votre personnage excelle…
				et dans lesquels il est moins à l’aise. Si tout cela vous paraît encore
			...
			<p/>
			<img  className="caracteristiques"
			...
			<div>
				<ul>
					...
					<li>
						<strong>Endurance (E)</strong> : indique votre résistance physique,
						votre résilience face aux poisons ou aux environnements difficiles.
					</li>

					<li>
						<strong>Initiative (I)</strong> : représente votre vitesse de pensée
						et de réaction.
					</li>
					...
					<li>
						<strong>Force Mentale (FM)</strong> : mesure votre volonté générale
						et votre capacité à ignorer la peur, la douleur ou la pression.
					</li>

					<li>
						<strong>Sociabilité (Soc)</strong> : votre aisance avec les autres,
						votre charme et votre capacité à paraître agréable ou convaincant.
					</li>
				</ul>
			</div>
			
				<p/>Maintenant que tout ceci est posé, revenons à notre histoire de dés et
				voyons un exemple :
				<strong>
					<p>Alphonse est docker sur les quais. Alphonse veut porter une caisse.</p>
				</strong><p/>

		<section className="image2">
				
				<p> Le MJ lui demande donc de faire un test de Force. Sa Force est de
				30. Il lance les dés et obtient 22. Ce résultat est inférieur à sa
				caractéristique : <strong> Alphonse réussit</strong> à porter la caisse
				jusqu’au dépôt. <br/><br/>
			
				Bien sûr, Alphonse peut être aidé, ou la difficulté peut varier selon la
				situation. Le maître du jeu peut juger qu’Alphonse, en tant que docker,
				fait cela tous les jours : il peut donc accorder un
				<strong> bonus de +40</strong> pour cette action spécifique. Alphonse ne
				teste plus sur 30, mais sur 70, ce qui rend la réussite beaucoup plus
				probable.<br/><br/>
			
				À l’inverse, si Alphonse tente une tâche difficile ou inhabituelle, un
				<strong> malus</strong> peut être appliqué à son test. Toutes ces nuances
				permettent d’identifier les points forts et faibles d’un personnage… et
				les compétences permettront d’aller encore plus loin dans la précision.
			</p>
			...
			<p>
				...
				manière précise. Elles couvrent une grande variété d’actions possibles.
				Certaines compétences appartiennent à une liste commune, tandis que
				d’autres, plus spécialisées, sont dites <strong> avancées</strong> — mais
				nous n’en parlerons pas ici afin de rester clairs et accessibles.{" "}
			</p>
			...
			<p>
				Reprenons Alphonse le docker. La compétence <em> Art</em> dépend de la
				caractéristique <strong> Dex</strong>. Créer une œuvre demande minutie,
				patience et finesse, des qualités qu’Alphonse ne possède pas vraiment :
				il est fort, rustre et peu éduqué concernant les arts. Il ne bénéficie
				d’aucune <strong> Augmentation (Aug)</strong> dans ce domaine. Ainsi, sa
				valeur totale en Art se limite à sa caractéristique de base — comme
				indiqué dans la colonne <strong> “Total”</strong>.
			<p/>
			
				Cependant, Alphonse a un passé un peu… mouvementé. Il a traîné avec de
				mauvaises fréquentations, appris l’usage de la poudre noire, et il se
			...
			<p/>
				...
				Au total, sa compétence atteint donc <strong> 60</strong>. S’il tire, il
				devra obtenir <strong> 60 ou moins</strong> sur son jet pour toucher sa
				cible.
			</p>
			...
			<p>
				...
				pas l’art, mais bien le maniement de son pistolet. Ce raisonnement vaut
				pour toutes les compétences :
				<strong>
					plus vous investissez dans une compétence, plus ses Augmentations
					(Aug) augmentent, et plus réaliser une action liée à celle-ci devient
					facile
				</strong>
				.
			</p>
			...
			<p>
				...
				<strong> dizaine</strong> de votre seuil de réussite. L’écart entre les
				deux indique à quel point votre action est réussie… ou ratée.
			</p>

			<section className="image4">

			<p>
				Reprenons Alphonse, cette fois dans une tentative de marchandage avec un
				vendeur. La compétence correspondante dépend de sa
				<strong> Sociabilité (Soc) de 30</strong>. Alphonse lance les dés et
				obtient <strong>87</strong>. L’échec est très net :
				<strong>5 dizaines d’écart</strong>.
			<p/>
			
				Une telle tentative maladroite peut mettre le marchand en colère : il
				peut l’insulter, hausser les prix, demander à son garde de «
				raccompagner » Alphonse, ou encore refuser toute transaction.
			<p/>
			
				Dans certaines situations, un autre joueur peut venir en soutien avant
				qu’un jet soit lancé. Par exemple : « Je décide d’aider Alphonse avec
				mon charme et mes conseils ». Dans ce cas, Alphonse gagne
				automatiquement <strong>+1 DR</strong> pour cette action. Cela augmente
				son score effectif de 30 à 40, rendant son test un peu plus simple.
			</p>
			...
			<p>
				...
				<strong> réussite critique</strong> et la <strong> maladresse</strong>. Si
				vous réussissez un jet et obtenez un double (11, 22, 33, etc.), il
				s’agit d’une réussite stupéfiante, quelle que soit la difficulté. À
				l’inverse, si vous échouez et obtenez un double (99, 88, 77, etc.), vous
			...
			<p>
				...
				sa
				<strong> Force Mentale</strong>, lutter lors d’un bras de fer grâce à sa
				...
				également en combat pour comparer les capacités martiales ou l’esquive.
			</p>
			...
			<p>
				Reprenons notre exemple : Alphonse doit maintenant se défendre face au
				garde du corps du marchand. Un combat débute. Tous les participants
				effectuent un test d’<strong> Initiative (I)</strong> pour déterminer
				l’ordre d’action. À chaque tour, chacun dispose de{" "}
				<strong> 1 mouvement</strong> et
				<strong> 1 action</strong>.
			<p/>

			
				Lors de la confrontation, le garde du corps tente de frapper Alphonse
				avec son épée (compétence de mêlée – base). Pour se défendre, Alphonse
				peut soit
			...
			<p>
				Le plus important est de vous amuser, d’interpréter votre personnage et
				de laisser l’histoire se dérouler. Les règles sont là pour vous
